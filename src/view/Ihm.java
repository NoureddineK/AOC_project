...
public class Ihm extends JFrame {
	...
	public static final int NB_AFFICHEUR = 6;
	...
	public Ihm() {
		...
		atomiqueBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				stopInst.execute();
				atomiquecmd = new algoAtomiqueCMD(generator);
				atomiquecmd.execute();
				startInst.execute();
			}
		...
		seqBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				stopInst.execute();
				seqcmd = new algoSequentielCMD(generator);
				seqcmd.execute();
				startInst.execute();
			}
		...
		causBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				stopInst.execute();
				epoquecmd = new algoEpoqueCMD(generator);
				epoquecmd.execute();
				startInst.execute();
			}
	...
	public void setCapteur(Generator generator) {
		...
		registerObservers((GeneratorImp) generator);
		startInst = new StartInst(generator);
		stopInst = new StopInst(generator);

	}

	private void registerObservers(GeneratorImp c) {
